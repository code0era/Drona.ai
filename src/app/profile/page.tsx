...
const ProfilePage = () => {
  // FIX 1: Destructure 'isLoaded' and 'isSignedIn' for reliable checks
  const { user, isLoaded, isSignedIn } = useUser();

  // FIX 2: Get the user's email address, which is now the foreign key
  const userEmailValue = user?.primaryEmailAddress?.emailAddress;

  // FIX 3: Implement the correct skip logic for useQuery
  // The query only runs if Clerk is loaded AND the required email is available.
  // The argument name MUST be 'userEmail' to match your plans.ts query definition.
  const queryArgs = (isLoaded && isSignedIn && userEmailValue)
    ? { userEmail: userEmailValue }
    : "skip";

  const allPlans = useQuery(
    api.plans.getUserPlans, // This query expects { userEmail: string }
    queryArgs
  );
  ...
  return (
    <section className="relative z-10 pt-12 pb-32 flex-grow container mx-auto px-4">
      ...
      {/* RENDER LOGIC: Check if plans are loaded and present */}
